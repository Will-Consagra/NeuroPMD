...
def vmf_kernel(W1, W2, B1, B2):
    if W1.shape[1] == 2:
        Wsum = W1[:, None, :] + W2[None, :, :]
        Wnorm = torch.norm(Wsum, dim=2)  
        J_0_Wnorm = torch.special.modified_bessel_i0(Wnorm)
        Bsum = B1 + B2.T 
        exp_fac = torch.exp(Bsum)
        return exp_fac * J_0_Wnorm
    elif W1.shape[1] == 3: # Currently only support data on sphere
